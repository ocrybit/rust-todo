...
fn reorder(todos: &mut Todos) -> Result<()> {
    println!("---------------------------------------");
	...
	for (ind, task) in todos.iter().enumerate() {
            if task.id == id {
		index = ind;
                break;
            }
...
fn command(todos: &mut Todos, prev: &mut String) -> Result<()> {
    println!("---------------------------------------");
    println!("enter command: 1) show, 2) add, 3) complete, 4) reorder, 5) del, 6) exit");
    println!("---------------------------------------");
    let mut buffer = String::new();
    stdin().read_line(&mut buffer)?;
...
fn main() -> Result<()> {
    ...
    let _ = exec(&mut todos, "s", &mut prev);
